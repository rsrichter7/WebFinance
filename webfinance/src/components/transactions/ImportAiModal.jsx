...
function buildPrompt(rows, allCats) {
  const toCat = rows
    ...
    .filter(r => r.selected && !r.categorie)
  ...
  const txList = toCat
    .map((r, i) => `${i + 1}. ${r.winkel || '(onbekend)'} | ${r.beschrijving || ''} | ${fmt(r.bedrag)} | ${r.type}`)
    .join('\n')
  ...
  const prompt = [
    ...
    txList,
    '',
    'Geef het resultaat als CSV (zonder headers), in dit formaat:',
    'regelnummer,categorie,subcategorie,soort',
    '',
    'Voorbeeld:',
    '1,Dagelijks leven,Boodschappen,Noodzaak',
    '2,Wonen,Verzekeringen (woning),Noodzaak',
    '3,Vrije tijd,Shoppen,Wens',
  ].join('\n')
...
function parseAiResult(text, toCat, allCats) {
  ...
  const lines = text
    .split('\n')
    .map(l => l.trim())
    .filter(l => l.length > 0 && !/^regelnummer/i.test(l))
  ...
  for (const line of lines) {
    ...
    if (!catObj || !catObj.subs.includes(subcategorie) || !SOORTEN_GELDIG.has(soort)) {
      skipped++
      continue
    }

    applied.push({
      index: toCat[num - 1]._origIndex,
      categorie,
      subcategorie,
      soort,
      _aiTagged: true,
...
export default function ImportAiModal({ open, onClose, rows, customCategories, onApply }) {
  ...
  function handleClose() {
    setCopied(false); setPasteText(''); setResultMsg('')
    onClose()
  }
  ...
  async function handleCopy() {
    try {
      ...
      setCopied(true) // toon success ook als fallback
    }
  }
  ...
  return (
    <div style={{ position: 'fixed', inset: 0, zIndex: 210, display: 'grid', placeItems: 'center' }}>
      <div onClick={handleClose} style={{ position: 'absolute', inset: 0, background: 'rgba(17,24,39,0.5)' }} />
      <div style={{
        position: 'relative', width: 'min(700px, 94vw)', maxHeight: '90vh',
        background: T.card, borderRadius: 14, boxShadow: '0 30px 80px rgba(0,0,0,0.25)',
