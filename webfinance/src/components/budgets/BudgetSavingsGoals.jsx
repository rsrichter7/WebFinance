    ...
    <div style={{
      ...
      <div style={{ position: 'relative', flex: 1, maxWidth: 140 }}>
        <span style={{
          ...
          fontSize: 13, color: T.ink3, pointerEvents: 'none',
        }}>€</span>
        <input
          type="text"
          inputMode="decimal"
          ...
          style={{
            ...
            fontFamily: 'inherit', ...TAB, outline: 'none',
            background: T.card,
          }}
        />
      </div>
      <button onClick={handleBevestig} style={{
        padding: '6px 12px', borderRadius: 6, border: 'none',
        background: T.blue, color: '#fff', fontSize: 12,
        fontWeight: 500, cursor: 'pointer', fontFamily: 'inherit',
      }}>
        Storten
      </button>
      <button onClick={onAnnuleer} style={{
        padding: '6px 10px', borderRadius: 6,
        border: `1px solid ${T.border}`, background: T.card,
        color: T.ink3, fontSize: 12, cursor: 'pointer', fontFamily: 'inherit',
      }}>
        Annuleer
      </button>
    </div>
...
function NieuwDoelForm({ onOpslaan, onAnnuleer }) {
  ...
  return (
    <div style={{
      padding: 16, borderTop: `1px solid ${T.rule}`,
        ...
        <div style={{ display: 'grid', gridTemplateColumns: '1fr 1fr', gap: 10 }}>
          <div>
            ...
            <input
              ...
              placeholder="Bijv. Vakantie 2027"
              autoFocus
              style={{
                width: '100%', padding: '8px 12px', borderRadius: 6,
                border: `1.5px solid ${T.border}`, fontSize: 13, color: T.ink,
                fontFamily: 'inherit', outline: 'none', background: T.card,
              }}
            />
          </div>
          <div>
            ...
            <div style={{ position: 'relative' }}>
              ...
              <input
                ...
                style={{
                  ...
                  border: `1.5px solid ${T.border}`, fontSize: 13, color: T.ink,
                  fontFamily: 'inherit', ...TAB, outline: 'none', background: T.card,
                }}
              />
            </div>
          </div>
...
export default function BudgetSavingsGoals({ spaardoelen, onToevoegen, onStorten, onVerwijderen }) {
  ...
  const handleStorten = (id, bedrag) => {
    onStorten(id, bedrag)
    setStortId(null)
  }

  const handleNieuwOpslaan = (doel) => {
    onToevoegen(doel)
    setToonNieuw(false)
  }
      ...
      <div style={{ display: 'flex', alignItems: 'center', justifyContent: 'space-between', marginBottom: 18 }}>
        ...
        <button
          ...
          style={{
            ...
            background: T.card, color: T.ink2, fontSize: 12, fontWeight: 500,
            cursor: 'pointer', fontFamily: 'inherit',
          }}
        >
        ...
        <div style={{ display: 'flex', flexDirection: 'column', gap: 0 }}>
          {spaardoelen.map((doel, i) => {
            ...
            return (
              <div
                key={doel.id}
                style={{ paddingBottom: 18, marginBottom: isLast && !toonNieuw ? 0 : 18, borderBottom: isLast && !toonNieuw ? 'none' : `1px solid ${T.rule}` }}
              >
                <div style={{ display: 'flex', alignItems: 'center', justifyContent: 'space-between', marginBottom: 10 }}>
                  <div style={{ display: 'flex', alignItems: 'center', gap: 8 }}>
                    ...
                    {doel.deadline && (
                      <Badge color={T.ink3} bg={T.rule}>{doel.deadline}</Badge>
                    )}
                  </div>
                  <div style={{ display: 'flex', alignItems: 'center', gap: 8 }}>
                    <span style={{ fontSize: 12, color: T.ink3, ...TAB }}>
                      {fmt(huidig)} / {fmt(doel.doelbedrag)}
                    </span>
                    <button
                      onClick={() => setStortId(stortId === doel.id ? null : doel.id)}
                      style={{
                        padding: '4px 10px', borderRadius: 6,
                        border: `1px solid ${stortId === doel.id ? T.blue : T.border}`,
                        background: stortId === doel.id ? T.blueSoft : T.card,
                    ...
                    <button
                      onClick={() => onVerwijderen && onVerwijderen(doel.id)}
                      style={{
                        padding: '4px 8px', borderRadius: 6,
                        border: `1px solid ${T.border}`, background: T.card,
                        color: T.ink4, fontSize: 11, cursor: 'pointer', fontFamily: 'inherit',
                      }}
                    >
                      ×
                    </button>
                ...
                <div style={{ display: 'flex', alignItems: 'center', gap: 10 }}>
                  ...
                  <span style={{ fontSize: 11, fontWeight: 600, color: T.blueText, ...TAB, minWidth: 34 }}>
                    {pct.toFixed(0)}%
                  </span>
                </div>
                {stortId === doel.id && (
                  <StortInput
                    onBevestig={(bedrag) => handleStorten(doel.id, bedrag)}
                    onAnnuleer={() => setStortId(null)}
                  />
                )}
              </div>
            )
