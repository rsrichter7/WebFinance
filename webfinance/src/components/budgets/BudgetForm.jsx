...
export default function BudgetForm({ open, editingItem, onClose, onSave, bestaandeCategorieën }) {
  ...
  // Categorieën die nog geen budget hebben (tenzij we bewerken)
  ...
  function updateSub(sub, value) {
    setSubBudgetten(prev => ({ ...prev, [sub]: value }))
  }
      ...
      <div style={{
        ...
        <div style={{ padding: '20px 24px', borderBottom: `1px solid ${T.border}`, display: 'flex', alignItems: 'center', justifyContent: 'space-between' }}>
          <div>
            <div style={{ fontSize: 17, fontWeight: 600, color: T.ink }}>
              {isEdit ? 'Budget bewerken' : 'Budget instellen'}
            </div>
            <div style={{ fontSize: 12, color: T.ink3, marginTop: 2 }}>
              {isEdit ? 'Wijzig het budget voor deze categorie' : 'Stel een budget in per categorie'}
            </div>
          </div>
        ...
        <div style={{ flex: 1, overflow: 'auto', padding: 24, display: 'flex', flexDirection: 'column', gap: 20 }}>
          ...
          {categorie && (
            <div style={{
              ...
              background: T[config.softKey], border: `1px solid ${T.border}`,
            }}>
              <div style={{
                width: 36, height: 36, borderRadius: 9,
                background: T.card, display: 'grid', placeItems: 'center',
            ...
            <div style={{
              ...
              {subs.map((sub, i) => (
                <div key={sub} style={{
                  display: 'flex', alignItems: 'center', gap: 10,
                  padding: '10px 14px',
                  borderBottom: i < subs.length - 1 ? `1px solid ${T.rule}` : 'none',
                  ...
                  <div style={{ position: 'relative', width: 120 }}>
                    ...
                    <input
                      ...
                      inputMode="decimal"
                      placeholder="0,00"
                      value={subBudgetten[sub] || ''}
                      onChange={e => updateSub(sub, e.target.value)}
                      style={{
                        width: '100%', padding: '7px 10px 7px 24px', borderRadius: 6,
                        border: `1.5px solid ${T.border}`, background: T.card,
                        fontSize: 13, color: T.ink, fontFamily: "'Inter', system-ui, sans-serif",
        ...
        <div style={{ padding: '16px 24px', borderTop: `1px solid ${T.border}`, display: 'flex', gap: 10 }}>
          <button onClick={handleSave} style={{
            ...
            fontSize: 13, fontWeight: 500, cursor: totaal > 0 ? 'pointer' : 'default',
            fontFamily: 'inherit',
          }}>
