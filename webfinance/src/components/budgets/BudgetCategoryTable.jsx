...
function SubRij({ sub, editing, editValue, onEditChange }) {
  ...
  return (
    <div style={{
      ...
      padding: '8px 22px 8px 52px', borderBottom: `1px solid ${T.rule}`,
        ...
        {editing ? (
          <div style={{ position: 'relative', width: 100 }}>
            ...
            <input
              ...
              inputMode="decimal"
              value={editValue}
              onChange={e => onEditChange(sub.naam, e.target.value)}
              placeholder="0,00"
              style={{
                width: '100%', padding: '5px 8px 5px 22px', borderRadius: 6,
                border: `1.5px solid ${T.blue}`, background: T.card,
                fontSize: 12, color: T.ink, fontFamily: "'Inter', system-ui, sans-serif",
                ...TAB, outline: 'none', textAlign: 'right',
              }}
            />
          </div>
        ) : (
          <span style={{ fontSize: 12, color: T.ink3, ...TAB }}>
            {sub.budget > 0 ? fmt(sub.budget) : '—'}
          </span>
        )}
      </div>
      <span style={{ textAlign: 'right', fontSize: 12, color: T.ink3, ...TAB }}>
        {fmt(sub.besteed)}
      </span>
      <div style={{ padding: '0 8px' }}>
        <ProgressBar pct={pct} size={5} />
      </div>
      <div style={{ display: 'flex', justifyContent: 'center' }}>
        {sub.budget > 0 && <PctBadge pct={Math.min(pct, 100)} />}
      </div>
      <span />
    </div>
  )
...
function CatRij({ item, onSave }) {
  ...
  function startEdit(e) {
    ...
    item.subs.forEach(sub => {
      values[sub.naam] = sub.budget > 0 ? String(sub.budget) : ''
    })
    setEditValues(values)
  }

  function handleEditChange(naam, value) {
    setEditValues(prev => ({ ...prev, [naam]: value }))
  }
    ...
    <>
      ...
      >
        <div style={{ display: 'flex', alignItems: 'center', gap: 8 }}>
          {hasSubs ? (
            <span style={{ color: T.ink4, display: 'flex' }}>
          ...
          <div style={{
            ...
            background: soft, color, display: 'grid', placeItems: 'center', flexShrink: 0,
          }}>
            {icon}
          </div>
          <span style={{ fontSize: 13, fontWeight: 500, color: T.ink }}>{item.categorie}</span>
        </div>
        <span style={{ textAlign: 'right', fontSize: 13, fontWeight: 500, color: T.ink, ...TAB }}>
        ...
        <div style={{ display: 'flex', justifyContent: 'center' }}>
          {item.budget > 0 && <PctBadge pct={Math.min(pct, 100)} />}
        </div>
        <div style={{ display: 'flex', justifyContent: 'center' }}>
          <span
            onClick={startEdit}
            style={{ color: editing ? T.blue : T.ink4, cursor: 'pointer', display: 'flex' }}
          >
            {ICONS.edit}
          </span>
        </div>
...
export default function BudgetCategoryTable({ categorieOverzicht, onSave, budgetLimiet, totaalCategorieBudget }) {
  ...
  return (
    <Card style={{ padding: 0, overflow: 'visible' }}>
      <div style={{ padding: '16px 22px 14px', borderBottom: `1px solid ${T.border}`, display: 'flex', justifyContent: 'space-between', alignItems: 'flex-start' }}>
        <div>
          ...
          <div style={{ fontSize: 12, color: T.ink3, marginTop: 2 }}>
            Klik op een categorie voor subcategorieën
          </div>
        </div>
        <div style={{ textAlign: 'right' }}>
          <div style={{ fontSize: 11, color: T.ink3 }}>Nog te verdelen</div>
          <div style={{
            fontSize: 14, fontWeight: 600, ...TAB,
            color: resterendBudget >= 0 ? T.green : T.red,
          }}>
            {fmt(resterendBudget)}
          </div>
        </div>
      ...
      <KopRij />
      {categorieOverzicht.map(item => (
        <CatRij key={item.categorie} item={item} onSave={onSave} />
      ))}
    </Card>
  )
}