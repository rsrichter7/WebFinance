      ...
      <div style={{ display: 'flex', flexDirection: 'column', gap: 8, marginBottom: 20 }}>
        {profiles.map(p => (
          <div key={p.id} style={{
            ...
            padding: '12px 16px', background: T.card, borderRadius: 10,
            border: `1px solid ${T.border}`, opacity: p.isGezamenlijk ? 0.6 : 1,
          }}>
            <div style={{
            ...
            {!p.isGezamenlijk && (
              <div style={{ display: 'flex', gap: 6 }}>
                <button onClick={() => setModal({ type: 'edit', profile: p })} style={iconBtn}>
                  {ICONS.edit}
                </button>
                <button
  ...
  return (
    <>
      ...
      <div style={{
        ...
        padding: 28, width: 360, zIndex: 201,
      }}>
        ...
        <div style={{ marginBottom: 24 }}>
          ...
          <div style={{ display: 'flex', gap: 8, flexWrap: 'wrap' }}>
            {PROFIEL_KLEUREN.map((k, i) => (
              <div key={i} onClick={() => setKleur(k)} title={k.label} style={{
                width: 32, height: 32, borderRadius: '50%', background: k.bg, cursor: 'pointer',
                display: 'grid', placeItems: 'center',
                outline: kleur === k ? `3px solid ${k.fg}` : '3px solid transparent',
                outlineOffset: 2,
              }}>
  ...
  return (
    <>
      ...
      <div style={{
        ...
        background: T.card, border: `1px solid ${T.border}`, borderRadius: 14,
        boxShadow: '0 20px 60px rgba(17,24,39,0.15)',
        padding: 28, width: 340, zIndex: 201,
      }}>
        ...
        <div style={{ display: 'flex', gap: 10 }}>
          <button onClick={onConfirm} style={{ ...primaryBtn, background: T.red }}>Verwijderen</button>
          <button onClick={onClose} style={secondaryBtn}>Annuleren</button>
        </div>
      </div>
    </>
