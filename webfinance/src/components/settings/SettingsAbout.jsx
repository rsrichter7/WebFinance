...
export default function SettingsAbout({ onAdminUnlock }) {
  ...
  return (
    <div>
      ...
      <div style={{ display: 'flex', alignItems: 'center', gap: 16, padding: 18, border: `1px solid ${T.border}`, borderRadius: 12, background: T.card, marginBottom: 22 }}>
        <div style={{ width: 56, height: 56, borderRadius: 12, background: T.ink, color: '#fff', display: 'grid', placeItems: 'center', fontSize: 28, fontWeight: 600, flexShrink: 0 }}>€</div>
        <div style={{ flex: 1 }}>
          <div style={{ fontSize: 15, fontWeight: 600, color: T.ink }}>Webfinance</div>
          <div
            onClick={handleVersionClick}
            style={{ display: 'inline-flex', alignItems: 'center', gap: 4, fontSize: 12, color: T.ink3, ...TAB, cursor: 'pointer', marginTop: 2, padding: '2px 6px', borderRadius: 4, background: clicks > 0 ? T.bg : 'transparent', userSelect: 'none' }}
          >
            v0.1.0
            {clicks > 0 && clicks < 5 && <span style={{ fontSize: 10, color: T.ink4 }}>· {clicks}/5</span>}
          </div>
          <div style={{ fontSize: 12, color: T.ink4, marginTop: 4 }}>Persoonlijk financieel beheer · lokaal-eerst</div>
        </div>
        <span style={{ fontSize: 11, fontWeight: 600, padding: '4px 10px', borderRadius: 999, background: T.greenSoft, color: T.greenText, letterSpacing: 0.3 }}>BÈTA</span>
      </div>

      {justUnlocked && (
        <div style={{ padding: '10px 14px', borderRadius: 10, marginBottom: 18, background: T.amberSoft, border: '1px solid #FDE68A', fontSize: 13, color: '#78350F', fontWeight: 500 }}>
      ...
      <div style={{ fontSize: 11.5, color: T.ink4, paddingTop: 16, borderTop: `1px solid ${T.rule}`, display: 'flex', justifyContent: 'space-between' }}>
