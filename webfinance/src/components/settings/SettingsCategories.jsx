...
export default function SettingsCategories() {
  ...
  function save(next) { setCustom(next); updateSetting('custom_categories', next) }
  function toggleExpanded(name) { setExpanded(e => ({ ...e, [name]: !e[name] })) }

  function addSub(catName) {
    const val = (newSub[catName] || '').trim()
    if (!val) return
    save({ ...custom, customSubs: { ...custom.customSubs, [catName]: [...(custom.customSubs[catName] || []), val] } })
    setNewSub(s => ({ ...s, [catName]: '' }))
  }

  function deleteSub(catName, subName) {
    save({ ...custom, customSubs: { ...custom.customSubs, [catName]: (custom.customSubs[catName] || []).filter(s => s !== subName) } })
  }

  function addCat() {
    const val = newCat.trim()
    if (!val) return
    save({ ...custom, customCats: [...custom.customCats, { name: val }] })
    setNewCat('')
  }

  function deleteCat(name) {
    const { [name]: _, ...restSubs } = custom.customSubs
    save({ ...custom, customCats: custom.customCats.filter(c => c.name !== name), customSubs: restSubs })
  }
        ...
        {allCats.map((cat, i) => {
          ...
          return (
            <React.Fragment key={cat.name}>
              <div style={{ display: 'flex', alignItems: 'center', gap: 10, padding: '12px 14px', background: T.card, borderBottom: (!isLast || isExp) ? `1px solid ${T.rule}` : 'none' }}>
                ...
                {!cat.isDefault && (
                  <button onClick={() => deleteCat(cat.name)} style={iconBtn}>{ICONS.trash}</button>
                )}
              </div>
              ...
              {isExp && (
                <>
                  {allSubs.map((sub, j) => (
                    <div key={j} style={{ display: 'flex', alignItems: 'center', gap: 10, padding: '8px 14px 8px 42px', background: T.cardAlt, borderBottom: `1px solid ${T.rule}` }}>
                      <span style={{ flex: 1, fontSize: 12.5, color: T.ink3 }}>{sub}</span>
                      {customSubs.includes(sub) && (
                        <button onClick={() => deleteSub(cat.name, sub)} style={iconBtn}>{ICONS.trash}</button>
                      )}
                    </div>
                  ))}
                  <div style={{ display: 'flex', gap: 8, padding: '8px 14px 8px 42px', background: T.cardAlt, borderBottom: !isLast ? `1px solid ${T.rule}` : 'none' }}>
                    <input value={newSub[cat.name] || ''} onChange={e => setNewSub(s => ({ ...s, [cat.name]: e.target.value }))}
