...
function FeedbackCard({ item, STATUS_CFG, expanded, onExpand, notitie, onNotitie, onBehandeld, onAfwijzen }) {
  const { T } = useTheme()
    ...
    <div style={{ padding: 16, border: `1px solid ${T.border}`, borderRadius: 10, background: T.card }}>
      ...
      {item.afbeeldingUrl && (
        <div style={{ marginBottom: 10 }}>
          <img src={item.afbeeldingUrl} alt="feedback" onClick={() => setImgOpen(true)}
            style={{ maxHeight: 80, borderRadius: 6, border: `1px solid ${T.border}`, cursor: 'pointer', objectFit: 'cover' }} />
          {imgOpen && (
            <div onClick={() => setImgOpen(false)} style={{ position: 'fixed', inset: 0, background: 'rgba(0,0,0,0.6)', zIndex: 200, display: 'flex', alignItems: 'center', justifyContent: 'center' }}>
      ...
      <div style={{ fontSize: 11.5, color: T.ink4, marginBottom: item.status === 'open' ? 12 : 0 }}>
        ...
        {item.adminNotitie && <span style={{ marginLeft: 12, color: T.ink3, fontStyle: 'italic' }}>Notitie: {item.adminNotitie}</span>}
      </div>

      {item.status === 'open' && (
        <div style={{ display: 'flex', flexDirection: 'column', gap: 8 }}>
          <textarea placeholder="Admin-notitie (optioneel)" rows={2} value={notitie} onChange={e => onNotitie(e.target.value)}
            style={{ width: '100%', padding: '7px 10px', borderRadius: 8, border: `1px solid ${T.border}`, fontSize: 12, color: T.ink, background: T.card, fontFamily: 'inherit', resize: 'vertical', boxSizing: 'border-box' }} />
          <div style={{ display: 'flex', gap: 8 }}>
            <button onClick={onBehandeld} style={{ flex: 1, padding: '7px 0', borderRadius: 8, border: 'none', background: T.greenSoft, color: T.greenText, fontSize: 13, fontWeight: 500, cursor: 'pointer', fontFamily: 'inherit' }}>✓ Behandeld</button>
            <button onClick={onAfwijzen} style={{ flex: 1, padding: '7px 0', borderRadius: 8, border: 'none', background: T.redSoft, color: T.redText, fontSize: 13, fontWeight: 500, cursor: 'pointer', fontFamily: 'inherit' }}>× Afwijzen</button>
          </div>
        </div>
      )}
    </div>
