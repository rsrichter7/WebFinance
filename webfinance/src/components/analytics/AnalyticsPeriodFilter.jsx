...
export function initPeriod() {
  const now = new Date()
  return {
    modus: 'maand', maand: now.getMonth(), kwartaal: Math.floor(now.getMonth() / 3), jaar: now.getFullYear(),
  }
}
...
export default function AnalyticsPeriodFilter({ value, onChange }) {
  ...
  const navBtn = {
    ...
    color: T.ink3, fontSize: 15, lineHeight: 1, fontFamily: 'inherit',
  }

  return (
    <div style={{ display: 'flex', alignItems: 'center', gap: 3 }}>
      <div style={{ display: 'inline-flex', padding: 2, background: T.bg, border: `1px solid ${T.border}`, borderRadius: 7 }}>
        {MODI.map(([key, lbl]) => {
          const active = value.modus === key
          return (
            <span key={key} onClick={() => onChange({ ...value, modus: key })} style={{
              padding: '3px 9px', fontSize: 11.5, fontWeight: 500, borderRadius: 5, cursor: 'pointer', userSelect: 'none',
      ...
      <span style={{ fontSize: 12, fontWeight: 500, color: T.ink, minWidth: 84, textAlign: 'center' }}>
        {periodeLabel(value)}
      </span>
      <button style={navBtn} onClick={() => onChange(navigate(value, 1))}>›</button>
    </div>
