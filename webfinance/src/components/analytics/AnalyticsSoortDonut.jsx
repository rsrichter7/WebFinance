            ...
            <div key={s.key}>
              <div style={{ display: 'flex', alignItems: 'center', justifyContent: 'space-between', marginBottom: 5 }}>
                ...
                <div style={{ display: 'flex', alignItems: 'center', gap: 7 }}>
                  ...
                  <span style={{ fontSize: 11, fontWeight: 600, padding: '2px 6px', borderRadius: 4, background: s.soft, color: s.color }}>{pct.toFixed(0)}%</span>
                </div>
              </div>
              <div style={{ position: 'relative' }}>
                <div style={{ height: 5, background: T.rule, borderRadius: 3, overflow: 'hidden' }}>
                  <div style={{ height: '100%', borderRadius: 3, width: `${Math.min(pct, 100)}%`, background: s.color, transition: 'width 0.35s ease' }} />
                </div>
                <div style={{ position: 'absolute', top: -2, bottom: -2, left: `${s.target}%`, width: 2, background: T.ink4, borderRadius: 1 }} />
              </div>
