...
function getUnits(allTransactions, period) {
  ...
  if (modus === 'jaar') {
    return Array.from({ length: 12 }, (_, m) => {
      const txs = allTransactions.filter(t => { const d = new Date(t.datum); return d.getFullYear() === jaar && d.getMonth() === m })
  ...
  return Array.from({ length: daysInMonth }, (_, i) => {
    const day = i + 1
    const txs = allTransactions.filter(t => { const d = new Date(t.datum); return d.getFullYear() === jaar && d.getMonth() === maand && d.getDate() === day })
  ...
  return (
    <div>
      ...
      <svg viewBox={`0 0 ${W} ${H}`} style={{ width: '100%', height: 'auto', fontFamily: FONT }}>
        ...
        <path d={`${makePath(incPts)} L${incPts[n-1].x},${PT+CH} L${incPts[0].x},${PT+CH} Z`} fill={T.green} opacity="0.07" />
        <path d={makePath(incPts)} fill="none" stroke={T.green} strokeWidth="2" strokeLinecap="round" strokeLinejoin="round" />
        {incPts.map((p, i) => dotR > 0 && (
          <circle key={i} cx={p.x} cy={p.y} r={dotR} fill={T.card} stroke={T.green} strokeWidth="1.5" />
        ))}

        <path d={makePath(expPts)} fill="none" stroke={T.red} strokeWidth="2" strokeLinecap="round" strokeLinejoin="round" />
        {expPts.map((p, i) => dotR > 0 && (
          <circle key={i} cx={p.x} cy={p.y} r={dotR} fill={T.card} stroke={T.red} strokeWidth="1.5" />
        ))}

        {units.map((u, i) => (i % step === 0 || i === n - 1) && (
          <text key={i} x={incPts[i].x} y={H - 4} textAnchor="middle" fontSize="9.5" fill={T.ink4} fontFamily={FONT}>
            {u.label}
          </text>
        ))}
      </svg>
