...
function ItemRow({ label, icon, iconColor, amount, amountColor, isLast }) {
  return (
    <div style={{
      ...
      <span style={{ fontSize: 13, fontWeight: 500, color: amountColor, ...TAB, flexShrink: 0, marginLeft: 8 }}>
        {amount}
      </span>
    </div>
  )
  ...
  if (!day) {
    return (
      <div style={{
        ...
        borderRadius: 12, boxShadow: T.shadow, padding: 18,
      }}>
    ...
    <div style={{
      ...
      {hasExpected && (
        <div style={{ marginBottom: hasActual ? 16 : 0 }}>
          <div style={{ fontSize: 11, fontWeight: 500, color: T.ink4, textTransform: 'uppercase', letterSpacing: 0.3, marginBottom: 4 }}>
            Verwacht
          </div>
          {allExpected.map((e, i) => (
            <ItemRow
              key={i}
              label={e.name}
              icon={e.income ? ICONS.arrUp : ICONS.clock}
        ...
        <div>
          ...
          {allActual.map((a, i) => (
            <ItemRow
              key={i}
              label={a.name}
              icon={a.income ? ICONS.arrUp : ICONS.arrDown}
      ...
      {!hasExpected && !hasActual && (
        <div style={{ fontSize: 13, color: T.ink4, padding: '12px 0' }}>
          Geen transacties op deze dag
        </div>
      )}
    </div>
  )
