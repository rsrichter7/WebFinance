...
// Geeft de maandag van de week voor een gegeven dag
...
export default function CalendarWeekView({
  ...
  const monday = useMemo(
    () => getMondayOfWeek(year, month, refDay),
    [year, month, refDay],
  )
  ...
  // Dagmaps per unieke (jaar, maand) in de week — max 2
            ...
            >
              ...
              {actItems.filter(a => !a.income).map((a, j) => (
                <div key={j} style={{ display: 'flex', alignItems: 'flex-start', gap: 4, marginBottom: 5 }}>
                  ...
                  <span style={{ fontSize: 10.5, color: T.ink2, fontWeight: 500, ...TAB, lineHeight: 1.3 }}>
                    {fmtShort(a.amount)}
                  </span>
                </div>
              ))}
